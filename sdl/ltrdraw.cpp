...
/*
  int iter=0;
    do{
      std::bitset<8> x(fontData[iter]);
      std::cout << x;
//    for (int i = sizeof(uint8_t) - 1; i >= 0; --i) {
//        std::cout << ((fontData[iter] >> i) & 1);
//    }
//    //std::cout << std::endl;
//    if(iter==2 || iter ==4 || iter==6 || iter==8){
//       std::cout<<"---------------------------------"<<std::endl;
//      }
      if((iter)%18==0){
        printf("\n");
...
//    
    ...
    */
    ...
    //
    //for (int i =0; i<256; i++){
    //  drawCharacter(i,renderer,((i)*(9*2+14))%640,(i/20)*(32+8),2);
    //}
    ...
    for (int j = 0; j<15;j++){
      ...
      for (int l = 0; l<WINDOW_WIDTH;l++){
              SDL_RenderDrawPoint(renderer,l,j*(32+8) +8);
      }
    }
    //drawCharacter(101,renderer, 0, 0,2);  
    */
